...
export default function AdminProductCreate() {
  ...
  const canSubmit = useMemo(() => Boolean(
    name.trim() && slug.trim() && description.trim().length >= 20 && category.trim() &&
      ...
      csv(packagingOptions).length > 0 && csv(customizationOptions).length > 0 && images.length >= 2
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { data: product, error: productError } = await client.from("products").insert({ slug: safeSlug, name: name.trim(), description: description.trim(), category: category.trim(), moq: Number(moq), colors: csv(colors), sizes: csv(sizes), packaging_options: csv(packagingOptions), customization_options: csv(customizationOptions), created_by: user?.uid }).select("id").single();
