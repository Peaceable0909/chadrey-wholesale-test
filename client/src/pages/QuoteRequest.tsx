...
import { ArrowLeft, ArrowRight, Check, ChevronDown, FileUp, Image as ImageIcon, Loader2, Menu, Plus, Trash2, X } from "lucide-react";
...
type ProductImage = { id: string; public_url: string; is_primary: boolean; sort_order: number; alt_text?: string | null; label?: string | null };
...
export default function QuoteRequest() {
  ...
  useEffect(() => {
    ...
    let active = true;
    void supabase.from("products").select("id,slug,name,category,moq,description,option_groups,colors,sizes,packaging_options,customization_options,product_images(id,public_url,is_primary,sort_order,alt_text,label)").eq("is_active", true).order("created_at", { ascending: false }).then(({ data, error }) => {
      if (!active) return;
  ...
  function selectReferenceFile(event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    const allowed = file.type.startsWith("image/") || ["application/pdf", "text/plain", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"].includes(file.type);
    if (!allowed || file.size > 10 * 1024 * 1024) return setSubmitError("Choose an image, PDF, DOC, DOCX, or TXT file up to 10 MB.");
    setSubmitError(""); setReferenceFile(file);
  }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (referenceFile) {
        const safeName = referenceFile.name.replace(/[^a-zA-Z0-9._-]/g, "-");
        const storagePath = `quote-references/${user.uid}/${request.id}/${Date.now()}-${safeName}`;
        const { error: uploadError } = await client.storage.from("quote-references").upload(storagePath, referenceFile, { contentType: referenceFile.type, upsert: false });
        if (uploadError) {
          await client.from("quote_requests").delete().eq("id", request.id);
          throw uploadError;
        }
        const { error: fileError } = await client.from("quote_request_files").insert({ quote_request_id: request.id, storage_path: storagePath, file_name: referenceFile.name, content_type: referenceFile.type || "application/octet-stream", size_bytes: referenceFile.size });
        if (fileError) {
          await client.storage.from("quote-references").remove([storagePath]);
          await client.from("quote_requests").delete().eq("id", request.id);
          throw fileError;
        }
      }
  ...
  if (submittedRef) return <div className="app-shell"><header className="site-header reference-header"><Link href="/" className="brand"><span className="brand-mark">C</span><span>CHADREY<small>WHOLESALE</small></span></Link></header><main className="confirmation-page quote-success-page" role="status" aria-live="polite"><div className="success-mark quote-success-mark" aria-hidden="true"><span className="success-ring"/><Check size={27}/></div><span className="eyebrow success-eyebrow">REQUEST RECEIVED</span><h1>Your quote request has been sent.</h1><p>Thank you. We have received your products, preferences, requirements, and any reference material. Our team will review everything and prepare your quotation.</p><div className="success-next-step"><strong>What happens next?</strong><span>We will contact you when your quotation is ready. You can track the request and its status from your workspace.</span></div><div className="reference-card"><span>Request reference</span><strong>{submittedRef}</strong><small>Track it from your workspace.</small></div><div className="hero-actions"><Link href="/dashboard" className="button button-primary">View my quotes <ArrowRight size={16}/></Link><Link href="/products" className="button button-secondary">Browse more products</Link></div></main><footer className="reference-footer"><div className="footer-brand"><span className="brand-mountain">⌃</span><div><strong>Chadrey Wholesale</strong><small>Your trusted partner for wholesale success.</small></div></div><div className="footer-socials"><Link href="/contact" aria-label="Contact Chadrey on LinkedIn">in</Link><Link href="/contact" aria-label="Contact Chadrey on Facebook">f</Link><Link href="/contact" aria-label="Contact Chadrey on Instagram">◎</Link><Link href="/contact" aria-label="Email Chadrey">✉</Link></div><div className="footer-bottom"><span>© 2026 Chadrey Wholesale. All rights reserved.</span></div></footer></div>;

  return <div className="app-shell"><header className="site-header reference-header"><Link href="/" className="brand"><span className="brand-mark">C</span><span>CHADREY<small>WHOLESALE</small></span></Link><nav className="main-nav"><Link href="/">Home</Link><Link href="/products">Products</Link><Link className="active" href="/quote">Request a Quote</Link><Link href="/dashboard">My Workspace</Link></nav><input type="checkbox" id="navToggleQuote" className="nav-toggle-checkbox" /><label htmlFor="navToggleQuote" className="mobile-menu-button" aria-label="Toggle navigation menu"><Menu size={20}/></label><Link href="/dashboard" className="avatar" title={user.name}>{initialsFrom(user.name, user.email)}</Link></header><main className="page-container narrow quote-page"><Link href="/products" className="back-link"><ArrowLeft size={15}/> Back to products</Link><div className="page-kicker"><span className="eyebrow">REQUEST A QUOTE</span><span className="step-progress"><i/><i/><i/></span></div><div className="quote-page-heading"><div><h1>Tell us exactly what you need.</h1><p className="page-lede">Choose the products, options, and image references you prefer. Then use the requirements box to describe anything else our sourcing team should know.</p></div><span className="quote-secure-note">Your request is saved to your workspace.</span></div><form onSubmit={submit}><section className="form-section"><div className="form-section-heading"><div><span className="eyebrow">01 / PRODUCT REQUIREMENTS</span><h2>Build your request</h2></div><button type="button" className="button button-secondary small" onClick={addLine} disabled={!products.length}><Plus size={15}/> Add another product</button></div>{lines.map((line, index) => { const product = productById.get(line.productId); const groups = product ? normalizeOptionGroups(product) : []; return <div className="quote-line enhanced-quote-line" key={line.id}><div className="quote-line-top"><span className="line-number">{String(index + 1).padStart(2, "0")}</span><div className="quote-line-title"><strong>{product?.name || "Choose a product"}</strong>{product && <span>{product.category} · MOQ {product.moq}</span>}</div>{lines.length > 1 && <button type="button" className="remove-line" onClick={() => setLines(current => current.filter(item => item.id !== line.id))}><Trash2 size={15}/> Remove</button>}</div><div className="form-grid"><label className="field wide">Product<select value={line.productId} onChange={event => { const next = productById.get(event.target.value); if (next) { const prefill = newLine(next); updateLine(line.id, { productId: next.id, quantity: next.moq, selections: prefill.selections, customValues: {}, preferredImageIds: prefill.preferredImageIds, requirements: "" }); } }}><option value="">Choose a live catalogue product</option>{products.map(item => <option key={item.id} value={item.id}>{item.name} · MOQ {item.moq}</option>)}</select></label><label className="field">Quantity<input type="number" min={product?.moq || 1} value={line.quantity} onChange={event => updateLine(line.id, { quantity: Math.max(product?.moq || 1, Number(event.target.value) || 1) })}/><small>{product ? `Minimum order: ${product.moq}` : "Select a product first"}</small></label>{groups.map(group => <label className="field" key={group.key}>{group.label}<select value={line.selections[group.key] || ""} onChange={event => updateSelection(line, group, event.target.value)}><option value="">Choose {group.label.toLowerCase()}</option>{group.options.map(option => <option key={option}>{option}</option>)}<option value="__other__">Other / not listed</option></select>{line.selections[group.key] === "__other__" && <input className="nested-option-input" value={line.customValues[group.key] || ""} onChange={event => updateCustomValue(line, group, event.target.value)} placeholder={`Enter ${group.label.toLowerCase()}`}/>}</label>)}{product && product.product_images.length > 0 && <div className="field wide preferred-images-field"><div className="field-heading"><span>Preferred product images</span><small>Select one or more views you prefer.</small></div><div className="preferred-image-grid">{product.product_images.map((image, imageIndex) => <button type="button" className={`preferred-image ${line.preferredImageIds.includes(image.id) ? "selected" : ""}`} aria-pressed={line.preferredImageIds.includes(image.id)} onClick={() => toggleImage(line, image.id)} key={image.id}><img src={image.public_url} alt={image.alt_text || `${product.name} view ${imageIndex + 1}`}/><span>{line.preferredImageIds.includes(image.id) ? <Check size={13}/> : <ImageIcon size={13}/>} {image.label || `View ${imageIndex + 1}`}</span></button>)}</div></div>}<label className="field wide requirements-field"><span>What should we know about this product?</span><small>Include dimensions, materials, branding, color breakdown, deadline, or anything not covered above.</small><textarea rows={5} value={line.requirements} onChange={event => updateLine(line.id, { requirements: event.target.value })} placeholder="Write the complete product-specific requirements here…"/></label></div></div>; })}</section><section className="form-section overall-requirements-section"><div className="form-section-heading"><div><span className="eyebrow">02 / YOUR BRIEF</span><h2>Tell us everything else</h2></div><ChevronDown size={20} color="var(--green)"/></div><label className="field wide requirements-field main-requirements-field"><span>Overall request requirements <strong>*</strong></span><small>Use this space for shared instructions, intended use, delivery destination, timing, target budget, and any request that applies to the full order.</small><textarea required minLength={10} rows={8} value={overallRequirements} onChange={event => setOverallRequirements(event.target.value)} placeholder="Example: We need a mixed wholesale order for a school campaign arriving before…"/></label></section><section className="form-section"><div className="form-section-heading"><div><span className="eyebrow">03 / REFERENCE MATERIAL</span><h2>Show us an example</h2></div><FileUp size={20} color="var(--green)"/></div><label className="reference-upload-field"><input type="file" accept="image/*,.pdf,.doc,.docx,.txt" onChange={selectReferenceFile}/><span><FileUp size={17}/> {referenceFile ? referenceFile.name : "Upload an additional image or file"}</span><small>Optional · Image, PDF, DOC, DOCX, or TXT · up to 10 MB</small></label>{referenceFile && <button type="button" className="remove-reference" onClick={() => setReferenceFile(null)}><X size={14}/> Remove reference file</button>}</section><section className="form-section"><div className="form-section-heading"><div><span className="eyebrow">04 / CONTACT AND DELIVERY</span><h2>Where should we send the quote?</h2></div></div><div className="form-grid"><label className="field">Full name<input required value={contact.name} onChange={event => setContact({ ...contact, name: event.target.value })} placeholder="Your name"/></label><label className="field">Company<input value={contact.company} onChange={event => setContact({ ...contact, company: event.target.value })} placeholder="Company or store name"/></label><label className="field">Work email<input required type="email" value={contact.email} onChange={event => setContact({ ...contact, email: event.target.value })} placeholder="you@company.com"/></label><label className="field">Phone number<input required value={contact.phone} onChange={event => setContact({ ...contact, phone: event.target.value })} placeholder="+234 …"/></label><label className="field">WhatsApp number<input value={contact.whatsapp} onChange={event => setContact({ ...contact, whatsapp: event.target.value })} placeholder="If different"/></label><label className="field wide">Delivery address<textarea required rows={3} value={contact.address} onChange={event => setContact({ ...contact, address: event.target.value })} placeholder="Street, city, state, country"/></label></div></section><section className="form-section quote-summary-section"><div className="form-section-heading"><div><span className="eyebrow">05 / REVIEW BEFORE SENDING</span><h2>Check your request</h2></div><Check size={20} color="var(--green)"/></div><div className="quote-summary-list">{lines.map((line, index) => { const product = productById.get(line.productId); const selectedImages = product?.product_images.filter(image => line.preferredImageIds.includes(image.id)) || []; return <article className="quote-summary-item" key={line.id}><div><span className="line-number">{String(index + 1).padStart(2, "0")}</span><strong>{product?.name || "Product not selected"}</strong><span>Quantity {line.quantity}</span></div><p>{Object.entries(line.selections).filter(([, value]) => value && value !== "__other__").map(([key, value]) => `${key}: ${value}`).join(" · ") || "No options selected"}{selectedImages.length ? ` · Images: ${selectedImages.map(image => image.label || "selected view").join(", ")}` : ""}</p><small>{line.requirements || "No product-specific requirements."}</small></article>; })}</div><div className="quote-summary-brief"><strong>Overall requirements</strong><p>{overallRequirements || "Not yet completed"}</p>{referenceFile && <small>Reference file: {referenceFile.name}</small>}</div></section>{submitError && <p className="form-error" role="alert">{submitError}</p>}<div className="form-submit-row"><span className="quote-form-footnote">We will review the request before sending pricing.</span><button className="button button-primary" type="submit" disabled={submitting || !products.length}>{submitting ? <><Loader2 className="spin" size={16}/> Sending request…</> : <>Submit quote request <ArrowRight size={16}/></>}</button></div></form></main><footer className="reference-footer"><div className="footer-brand"><span className="brand-mountain">⌃</span><div><strong>Chadrey Wholesale</strong><small>Your trusted partner for wholesale success.</small></div></div><div className="footer-socials"><Link href="/contact" aria-label="Contact Chadrey on LinkedIn">in</Link><Link href="/contact" aria-label="Contact Chadrey on Facebook">f</Link><Link href="/contact" aria-label="Contact Chadrey on Instagram">◎</Link><Link href="/contact" aria-label="Email Chadrey">✉</Link></div><div className="footer-bottom"><span>© 2026 Chadrey Wholesale. All rights reserved.</span></div></footer></div>;
}
