...
import {Edit3, Image as ImageIcon, Loader2, PackagePlus, Save, Search, Trash2, X, ArrowRight, ArrowLeftToLine, ShieldCheck} from "lucide-react";
...
type ProductImage = { id: string; storage_path: string; public_url: string; is_primary: boolean; sort_order: number; label: string | null };
...
export default function AdminProducts() {
  ...
  function beginEdit(product: Product) {
    ...
    setImageLabels(Object.fromEntries(product.product_images.map(image => [image.id, image.label || ""])));
    setStatus(""); setError("");
  }
  ...
  async function saveEdit() {
    ...
    if (updateError) setError(updateError.message);
      ...
      const imageUpdates = await Promise.all(editing.product_images.map(image => getSupabaseClient().from("product_images").update({ label: imageLabels[image.id]?.trim() || null, alt_text: imageLabels[image.id]?.trim() || `${form.name.trim()} product image` }).eq("id", image.id)));
  ...
  if (!user) return <div className="confirmation-page admin-gate"><Link href="/" className="back-link"><ArrowLeftToLine size={14}/> Back to Chadrey Wholesale</Link><span className="success-mark neutral"><ShieldCheck size={26}/></span><span className="eyebrow">PRODUCT DIRECTORY</span><h1>Admin sign-in required.</h1><p>Use your Supabase administrator account to access the operations workspace. If you don't have access yet, ask a Chadrey Wholesale administrator to add you.</p><Link href="/admin/login" className="button button-primary">Sign in to operations <ArrowRight size={16}/></Link></div>;
